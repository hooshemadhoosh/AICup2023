...
def game_finished(main_game, score):
    ...
    with open("log/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S.%f") + ".json", "w") as log_file:
    ...
    with open("result_log/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S.%f") + ".json", "w") as result_log_file: 
    ...
    if main_game.debug:
        ...
        with open("debug_log/" + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S.%f") + ".txt", "w") as debug_log_file:
